...
class MailTmManager:
    ...
    def fetch_otp_garena(self, timeout=60, delay=3):
        """
        NÂNG CẤP: Quét hòm thư liên tục, làm sạch HTML và trả về thẳng mã OTP 6 số.
        """
        print(f"⏳ Đang đợi OTP Garena gửi về {self.email}...")
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if res.status_code == 200:
                    ...
                    # reversed để ưu tiên đọc thư mới nhất đổ về
                    for msg in reversed(messages):
                        sender = msg.get('from', {}).get('address', '').lower()
                        subject = msg.get('subject', '').lower()
                        
                        if "garena" in sender or "verification" in subject or "garena" in subject:
                            msg_id = msg.get('id')
                            ...
                            if detail_res.status_code == 200:
                                ...
                                # Chuẩn hóa text đầu vào chống lệch kiểu dữ liệu
                                ...
                                if match:
                                    otp = match.group(0)
                                    print(f"✅ Đã cào thành công OTP: {otp}")
                                    return otp
            except Exception as e:
                print(f"⚠️ Cảnh báo quét mail: {e}")
                
            time.sleep(delay)
            
        print("❌ Quá thời gian chờ (Timeout) — Không nhận được OTP.")
        return None
